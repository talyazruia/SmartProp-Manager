...
import * as XLSX from "xlsx"; // ייבוא ספריה לייצוא אקסל

//  פלטת צבעים מבוססת לוגו Smart-Prop ושפה עיצובית אחידה
const LOGO_BLUE = "#1a5f9e";   // הכחול הראשי של הלוגו
const LOGO_GOLD = "#f2b819";   // הצהוב/זהב של הברק בלוגו
const TEXT_DARK = "#2c3e50";   // צבע טקסט כהה ומודרני
...
const styles = {
  ...
  backBtn: {
    backgroundColor: "#546e7a", // צבע ניטרלי מקצועי לחזרה/ביטול
    color: "white",
    padding: "10px 20px",
    border: "none",
    borderRadius: "4px",
...
export default function LandlordDashboard({ setScreen, user }) {
  ...
  const handleGoToElectricity = async () => {
    try {
      const res = await axios.get("http://localhost:8081/api/electricity/price");
      setPreviousElectricityPrice(res.data.settingValue || 0);
      setElectricityPrice(res.data.settingValue || 0);
      setView("electricity");
    } catch (err) { setView("electricity"); }
  };

  const updateElectricityPrice = async () => {
    try {
      await axios.put("http://localhost:8081/api/electricity/price", { settingValue: Number(electricityPrice) });
      alert("מחיר חשמל עודכן בהצלחה! ✔️");
      setView("main");
    } catch (err) { alert("שגיאה בעדכון מחיר חשמל"); }
  };

  const handleGoToPaymentDetails = async () => {
    try {
      const res = await axios.get(`http://localhost:8081/api/landlords/${user.username}`);
      if (res.data) setPaymentDetails({
        bankAccountNumber: res.data.bankAccountNumber || "",
        bankBranch: res.data.bankBranch || "",
        bankName: res.data.bankName || ""
      });
      setView("paymentDetails");
    } catch (err) { setView("paymentDetails"); }
  };
  ...
  const updatePaymentDetails = async () => {
    try {
      ...
      setView("main");
    } catch (err) { alert("שגיאה בעדכון פרטים"); }
  };
  ...
  const handleRentApartment = async (id) => {
    ...
    } catch (err) { alert("שגיאה בשיוך שוכר"); }
  };
  ...
  const handleVacateApartment = async (id) => {
    ...
    } catch (err) { alert("שגיאה בפינוי הדירה"); }
  };
          ...
          <tbody>
            {paymentHistory.length > 0 ? paymentHistory.map(pay => (
              <tr key={pay.id}>
                ...
                <td style={styles.td}>{pay.amount} ₪</td>
                <td style={styles.td}>{pay.meterReading}</td>
                <td style={styles.td}>{pay.tenantUsername}</td>
              </tr>
            )) : <tr><td colSpan="4" style={{...styles.td, textAlign: 'center'}}>אין היסטוריית תשלומים לנכס זה</td></tr>}
          </tbody>
        </table>
      </div>
    );
                ...
                <td style={styles.td}>
                  {rentingAptId === apt.id ? (
                    <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                        <input type="text" value={tenantNameInput} placeholder="שם שוכר" onChange={(e) => setTenantNameInput(e.target.value)} style={{ width: "90px", padding: "4px" }} />
                        ...
                        <button onClick={() => setRentingAptId(null)} style={{ cursor:'pointer', backgroundColor: '#757575', color: 'white', border: 'none', padding: '4px 8px', borderRadius: '3px', fontWeight: 'bold' }}>ביטול</button>
                    </div>
                  ) : (rented ? apt.tenant : "—")}
                </td>
                <td style={styles.td}>
                  ...
                  <button style={{ ...styles.actionBtn, backgroundColor: "#c62828", color: "white" }} onClick={() => {}}> מחק</button>
                </td>
              </tr>
            );
          })}
