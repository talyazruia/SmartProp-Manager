...
const Tenant = ({ setScreen, setUser }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        // 2. יצירת ה-payload ושיוך האסוציאציה: המייל נכנס כשם המשתמש בדאטאבייס
        const payload = {
          firstName: values.firstName,
          lastName: values.lastName,
          phone: values.phone,
          email: values.email,
          username: values.email, // כאן המייל מוזרק לעמודת ה-username
        ...
        // 3. עדכון הסטייט הגלובלי של המשתמש המחובר עם האימייל בתור ה-username שלו
        setUser({
    ...
    <div style={styles.container}>
      ...
      <div style={styles.pageContainer}>
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          {/* 4. שורת "שם משתמש" הוסרה לחלוטין מה-JSX */}
