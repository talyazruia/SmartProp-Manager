...
export default function TenantDetails({ setScreen, user }) {
  ...
  if (view === "history") {
    return (
      <div style={{ ...styles.container, maxWidth: '800px' }}>
        ...
        <h2 style={{ textAlign: 'right', borderBottom: `2px solid ${LOGO_BLUE}`, paddingBottom: '10px', color: '#2c3e50' }}>היסטוריית תשלומים מלאה</h2>
        <table style={styles.table}>
          ...
          <tbody>
            ...
            )) : <tr><td colSpan="3" style={{ ...styles.td, textAlign: "center" }}>טרם בוצעו תשלומים</td></tr>}
          </tbody>
        </table>
      </div>
    );
  ...
  return (
    <div style={styles.container}>
      ...
      <button style={styles.uploadButton} onClick={() => setScreen("upload")}> העלאת קריאת מונה חדשה</button>
      <button style={styles.secondaryBtn} onClick={() => setView("history")}> הצג היסטוריית תשלומים וייצוא</button>
      ...
      <div style={{ marginTop: "45px" }}>
        <button style={{ 
          ...
          onClick={() => setScreen("login")}> התנתק</button>
      </div>
    </div>
  );
}